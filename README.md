...
This Python package helps manage and configure the **Windows PATH environment variable** for use in **MSYS2** and bash shells. It retrieves both **user and system PATH values** from the Windows Registry, splits them into logical groups (e.g., Python, pip), and converts them into **MSYS2-style paths** using `cygpath`.
...
* Filtering and categorizing directories (prepend and append)
* Converting Windows paths to MSYS2 format
* Automatically updating `~/.bashrc` with PATH adjustments
